      ...
      this._apiServices
        ...
        .subscribe(
          (responseInfo) => {
            ...
            if (this.language == 'ar') {
              for (var ingre of this.recipeDetails.extendedIngredients) {
                this.ingredients.push(ingre.original);
              }
              for (var inst of this.recipeDetails.analyzedInstructions[0]
                .steps) {
                this.instructions.push(inst.step);
              }
              this.lunchSummary.push(
                this.recipeDetails.dishTypes[0],
                this.recipeDetails.summary,
                this.recipeDetails.title
              );
              ...
              this.google.translate(googleObj1).subscribe(
                (res: any) => {
                  console.log(res);
                  this.translatedIngredients = res.data;
                },
                (err) => {
                  console.log(err);
                }
              ...
              this.google.translate(googleObj2).subscribe(
                (res: any) => {
                  console.log(res);
                  this.translatedInstructions = res.data;
                },
                (err) => {
                  console.log(err);
                }
              );
              const googleObj3: GoogleObj = {
                q: this.lunchSummary,
                target: 'ar',
              };
              this.google.translate(googleObj3).subscribe(
                (res: any) => {
                  console.log(res);
                  this.translatedLuSummary = res.data;
                },
                (err) => {
                  console.log(err);
                }
